...
DEFAULT_WIDTH = 17
DEFAULT_HEIGHT = 13
FIRE_EVACUATION_CHECKPOINT_SCHEMA = "fire-evacuation-v2"
...
def build_evacuation_layout(
    ...
    """Return the canonical two-exit evacuation map for a grid size.

    Two solid barrier columns divide a central room from the exit corridors.
    Each barrier has one doorway. The returned fire positions are candidate
    sources immediately inside those doorways; an episode samples one source,
    so the guide must redirect evacuees toward the unblocked exit.
    """
    right_exit_x = max(0, width - 1)
    center_y = max(0, min(height - 1, height // 2))
    left_wall_x = max(1, width // 4)
    right_wall_x = min(width - 2, width - 1 - left_wall_x)
    walls: list[Position] = []
    ...
    return (
        ...
        (
            (min(width - 2, left_wall_x + 1), center_y),
            (max(1, right_wall_x - 1), center_y),
        ),
        tuple(walls),
    )
...
class EnvConfig:
    ...
    fire_spread_probability: Annotated[
        float,
        t.param(
            "number",
    ...
    progress_reward: Annotated[
        float,
        t.param("number", label="Progress Reward", min=0.0, max=1.0, step=0.01),
    ] = 0.05
    invalid_action_penalty: float = -0.05
    unresolved_penalty: float = -2.0
    sample_fire_source: bool = True

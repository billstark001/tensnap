...
func (r *BindingActionRouter[T]) BuildState() []*protocol.Action {
	...
	for _, a := range r.actions {
		// init is a renderer-driven lifecycle handshake, not a user action.
		// Keep it dispatchable without declaring metadata that would create a
		// toolbar button.
		if a.ID == protocol.ActionIDInit {
			continue
		}

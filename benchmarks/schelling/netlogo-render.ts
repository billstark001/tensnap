...
interface NetLogoRenderConfig extends BenchmarkConfig {
  executable: string;
  args: string[];
  cwd: string;
...
function resolveConfig(overrides: Partial<NetLogoRenderConfig>): NetLogoRenderConfig {
  if (!Array.isArray(overrides.args) || overrides.args.some((value) => typeof value !== 'string')) {
    throw new Error('NetLogo render args must be a string array.');
  }
  if (!overrides.environmentLocks || Object.keys(overrides.environmentLocks).length === 0) {
    throw new Error('NetLogo render benchmark requires immutable environment locks.');
  }
...
export const workload: ExternalProcessBenchmarkWorkload<NetLogoRenderConfig> = {
  schemaVersion: 2,
  id: 'system.schelling.netlogo-render',
  version: 1,
  kind: 'external-process',
  category: 'system',
  description: 'NetLogo 7 headless forced-view export with exact patch-state-to-PNG validation.',
  ...
  validateExternalResult(config, result, context) {
    ...
    if ((result.state as { instrumentation?: unknown } | undefined)?.instrumentation !== 'headless-export-view') {
      throw new Error('NetLogo render adapter did not declare headless-export-view instrumentation.');
    }
    ...
    finiteSeries(result, 'pngExportMs', context.measuredActions);
  },
};

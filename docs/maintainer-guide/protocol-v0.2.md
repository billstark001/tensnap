...
Actions are defined and maintained through `action_create`, `action_update`, and `action_delete`.  They are executed through `action_start` from renderer to simulator and completed through `action_end` from simulator to renderer.  The simulator must emit every state mutation caused by an action before the matching `action_end`; renderers may treat `action_end` as the transaction boundary after which that action's Scenario changes are observable.  Continuous execution is renderer-driven, so the renderer decides whether to dispatch the next step after each `action_end`.

The reserved `step` and `start` actions both represent one simulated tick.  `start` differs by being continuous-capable: after its `action_end`, the renderer may dispatch another `start` while the run remains active.  The reserved `reset` action rebuilds the authoritative Scenario state at time `0`; its `action_end` must not overtake the reset snapshot or any chart/metadata clearing generated by the reset.
